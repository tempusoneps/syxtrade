...
# print(signals_data[signals_data.momentum_signal != ""])
# exit()
    ...
    def next(self):
        ...
        if not self.position:
            if ema_signal == SignalConfig.BUY_SIGNAL:
                ...
                self.sell(size=1, sl=sl, tp=tp)
                self.strategy = 'couple_cs'
            # elif macd_signal == SignalConfig.BUY_SIGNAL:
            #     buy_price = close_price
            #     sl1 = buy_price - (buy_price * self.max_sl)
            #     sl2 = self.data.prev_Low[-1]
            #     sl = max(sl1, sl2)
            #     tp = buy_price + (buy_price - sl) * self.RR
            #     self.buy(size=1, sl=sl, tp=tp)
            #     self.strategy = 'macd'
            # elif macd_signal == SignalConfig.SELL_SIGNAL:
            #     sell_price = close_price
            #     sl1 = sell_price + (sell_price * self.max_sl)
